# PlanetDiag - Gestionnaire de Cartes Réseau
# Usage:
#   -Action list                    → liste tous les adaptateurs réseau (JSON)
#   -Action reset -AdapterName <n>  → désactive puis réactive l'adaptateur (JSON)
#
...
# Validation du nom d'adaptateur (autorise lettres, chiffres, tirets, espaces, parenthèses)
function Test-AdapterName {
    param([string]$Name)
    if ([string]::IsNullOrWhiteSpace($Name)) { return $false }
    if ($Name.Length -gt 256)               { return $false }
    # Interdit les caractères dangereux pour PowerShell / injection
...
if ($Action -eq "reset") {
    ...
    try {
        # Vérifier l'existence de l'adaptateur (par nom exact)
        $adapter = Get-NetAdapter -Name $AdapterName -ErrorAction Stop

        # 1. Désactivation
        $steps.Add("Désactivation de '$AdapterName'…")
        Disable-NetAdapter -Name $AdapterName -Confirm:$false -ErrorAction Stop

        # Attendre que l'adaptateur soit désactivé (max 15s)
        ...
        $steps.Add("Adaptateur désactivé (statut : $statusAfterDisable).")

        # 2. Réactivation
        $steps.Add("Réactivation de '$AdapterName'…")
        ...
        $steps.Add("Adaptateur réactivé (statut : $finalStatus).")

    } catch {
        $success  = $false
        $errorMsg = $_.Exception.Message
        $steps.Add("ERREUR : $errorMsg")
        # Tentative de réactivation même en cas d'erreur
        try {
